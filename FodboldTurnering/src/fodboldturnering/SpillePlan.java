...
public class SpillePlan {
    ...
    public SpillePlan(ArrayList<Klub> holdListe, ArrayList<Turneringsrunde> rundeListe) {
        ...
        for (int i = 0; i < holdListe.size(); i++) {
            for (int j = i + 1; j < holdListe.size(); j++) {
                frieKampe.add(new Kamprapport(holdListe.get(i), holdListe.get(j), null));
                frieKampe.add(new Kamprapport(holdListe.get(j), holdListe.get(i), null));
            }
        }
        ...
        while (!frieKampe.isEmpty() && count<1000) {
            Turneringsrunde tr = new Turneringsrunde(rundenummer);
            rundenummer++;
